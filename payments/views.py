...
def payment_list(request):
    ...
    if search_form.is_valid():
        ...
        if customer:
            payments = payments.filter(invoice__customer__name__icontains=customer)
    ...
    if hasattr(request.user, "userprofile"):
        if request.user.userprofile.is_trader:
            payments = payments.filter(invoice__sale__assigned_trader=request.user)
...
def payment_create(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            payment.save()
            messages.success(
                request, f"Paiement {payment.payment_number} enregistré avec succès."
            )
    ...
    return render(
        request, "payments/form.html", {"form": form, "title": "Nouveau Paiement"}
    )
...
def payment_detail(request, pk):
    payment = get_object_or_404(
        Payment.objects.select_related(
            ...
            # Fetch all vehicles in the sale so templates can iterate them
            "invoice__sale__line_items__vehicle",
        ),
        pk=pk,
    )
    ...
    if hasattr(request.user, "userprofile"):
        if request.user.userprofile.is_trader:
            if payment.invoice.sale.assigned_trader != request.user:
                messages.error(
                    request,
                    "Vous ne pouvez voir que les paiements de vos propres ventes.",
                )
...
def payment_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            payment.save()
            messages.success(
                request, f"Paiement {payment.payment_number} modifié avec succès."
            )
            return redirect("payments:detail", pk=payment.pk)
    else:
...
def outstanding_invoices(request):
    invoices = (
        Invoice.objects.filter(balance_due__gt=0)
        .select_related("customer", "sale__assigned_trader")
    ...
    total_overdue = (
        overdue_invoices.aggregate(Sum("balance_due"))["balance_due__sum"] or 0
    )
    ...
    stats = {
        ...
        "avg_outstanding": (
            total_outstanding / invoices.count() if invoices.count() > 0 else 0
        ),
    }
...
def quick_payment(request, invoice_id):
    ...
    return render(
        request,
        "payments/quick_payment_form.html",
...
def payment_plan_create(request, invoice_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Plan de paiement créé avec {plan.number_of_installments} échéances.",
            )
    ...
    return render(
        request,
        "payments/payment_plan_form.html",
        {
            "form": form,
            "invoice": invoice,
...
def payment_plan_detail(request, pk):
    ...
    stats = {
        ...
        "completion_percentage": (
            (paid_installments.count() / installments.count() * 100)
    ...
    return render(
        request,
        "payments/payment_plan_detail.html",
        {"plan": plan, "installments": installments, "stats": stats},
    )
...
def installment_payment(request, installment_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if amount <= 0 or amount > float(installment.balance_due):
