...
class Payment(BaseModel):
    ...
    bank_reference = models.CharField(
        max_length=100, blank=True, verbose_name="Référence bancaire/chèque"
    )
    ...
    def update_invoice_balance(self):
        """Recalculate invoice totals from all confirmed payments."""
        ...
        total_payments = invoice.payments.filter(is_confirmed=True).aggregate(
...
class PaymentReminder(BaseModel):
    ...
    reminder_type = models.CharField(
        max_length=20, choices=REMINDER_TYPES, verbose_name="Type de relance"
    )
    message = models.TextField(verbose_name="Message")
    sent_by = models.ForeignKey(
        User, on_delete=models.PROTECT, verbose_name="Envoyé par"
    )

    customer_response = models.TextField(blank=True, verbose_name="Réponse du client")
    follow_up_date = models.DateField(
        null=True, blank=True, verbose_name="Date de suivi"
    )
...
class PaymentPlan(BaseModel):
    ...
    total_amount = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Montant total (DA)"
    )
    ...
    remaining_amount = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Montant restant (DA)"
    )
    ...
    installment_amount = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Montant par échéance (DA)"
    )

    start_date = models.DateField(verbose_name="Date de début")
    status = models.CharField(
        max_length=20, choices=PLAN_STATUS, default="active", verbose_name="Statut"
    )
    notes = models.TextField(blank=True, verbose_name="Notes")
    ...
    def save(self, *args, **kwargs):
        # Capture whether this is a new record BEFORE calling super()
        is_new = self._state.adding

        self.remaining_amount = self.total_amount - self.down_payment
        if self.number_of_installments > 0:
            self.installment_amount = (
                self.remaining_amount / self.number_of_installments
            )

        super().save(*args, **kwargs)

        # Only create installments once, when the plan is first created
        if is_new:
            self.create_installments()

    def create_installments(self):
        """Create individual installment records (monthly schedule)."""
...
class Installment(BaseModel):
    ...
    amount = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Montant (DA)"
    )
    ...
    def __str__(self):
        return (
            f"Échéance {self.installment_number} "
            f"- {self.payment_plan.invoice.invoice_number}"
        )
