...
from core.decorators import manager_required, trader_required
...
def my_commission(request):
    ...
    if not hasattr(request.user, 'userprofile') or not request.user.userprofile.is_trader:
    ...
    for i in range(6):
        ...
        performance_data.append({
            ...
            'commission': month_sales.aggregate(Sum('commission_amount'))['commission_amount__sum'] or 0,
        })
    ...
    if current_stats['sales_count'] > 0:
        applicable_tier = CommissionTier.objects.filter(
            is_active=True,
            min_sales_count__lte=current_stats['sales_count']
        ).filter(
...
def commission_overview(request):
    ...
    outstanding_total = outstanding_summaries.aggregate(
...
def trader_performance(request):
    ...
    for trader in traders:
        ...
        avg_commission_rate = (total_commission / total_margin * 100) if total_margin > 0 else 0
...
def commission_tier_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"Niveau de commission '{tier.name}' créé avec succès.")
            return redirect('commissions:tiers')
    else:
        form = CommissionTierForm()
    
    return render(request, 'commissions/tier_form.html', {
        'form': form,
        'title': 'Nouveau Niveau de Commission'
    })
...
def commission_tier_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"Niveau de commission '{tier.name}' modifié avec succès.")
            return redirect('commissions:tiers')
    else:
...
def commission_adjustment_create(request, summary_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, "Ajustement de commission enregistré avec succès.")
    ...
    return render(request, 'commissions/adjustment_form.html', {
        'form': form,
        'summary': summary,
...
def commission_payment_create(request, summary_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"Paiement de commission enregistré pour {summary.trader.get_full_name()}.")
            return redirect('commissions:overview')
    else:
        form = CommissionPaymentForm(summary=summary)
    
    return render(request, 'commissions/payment_form.html', {
        'form': form,
        'summary': summary,
...
def close_commission_period(request, year, month):
    ...
    if request.method == 'POST':
        ...
        if period.is_closed:
            return JsonResponse({
                'success': False,
                'message': 'Cette période est déjà fermée.'
            })
...
def ajax_commission_calculation(request):
    ...
    try:
        ...
        if sales_count > 0:
            applicable_tier = CommissionTier.objects.filter(
                is_active=True,
                min_sales_count__lte=sales_count
            ).filter(
