...
def vehicle_list(request):
    ...
    expired_reservations = vehicles.filter(
        status='reserved',
        reservation_expires__lt=timezone.now()
    )
...
def vehicle_detail(request, pk):
    ...
    vehicle = get_object_or_404(
        Vehicle.objects.select_related(
            ...
            'vehicle_purchase__currency',
            'vehicle_purchase__freight_cost',
            'vehicle_purchase__customs_declaration',
            'reserved_by'
        ).prefetch_related('photos'),
        pk=pk
    )
    ...
    if hasattr(purchase, 'freight_cost'):
        cost_breakdown['freight_cost'] = purchase.freight_cost.total_freight_cost_da or 0
    
    if hasattr(purchase, 'customs_declaration'):
        cost_breakdown['customs_cost'] = purchase.customs_declaration.total_customs_cost_da or 0
    ...
    can_reserve = (
        ...
        (request.user.userprofile.is_trader or request.user.userprofile.is_manager) and
        vehicle.status == 'available'
    )
    
    # Check if user can release reservation
    can_release = (
        vehicle.status == 'reserved' and
...
def vehicle_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            purchase = vehicle.vehicle_purchase
            if hasattr(purchase, 'customs_declaration') and purchase.customs_declaration.is_cleared:
                vehicle.status = 'available'
        ...
        if form.is_valid():
            ...
            try:
                ...
                return JsonResponse({
                    ...
                    'expires': vehicle.reservation_expires.strftime('%d/%m/%Y %H:%M')
                })
...
def vehicle_add_photo(request, pk):
    ...
    return render(request, 'inventory/photo_form.html', {
        'form': form,
        'vehicle': vehicle,
...
def stock_alerts(request):
    ...
    alerts = StockAlert.objects.filter(is_resolved=False).select_related('vehicle', 'created_by')
...
def generate_automatic_alerts():
    ...
    for vehicle in slow_vehicles:
        if vehicle.is_slow_moving and not StockAlert.objects.filter(
