...
def dashboard(request):
    ...
    financial_stats = {
        'monthly_revenue': monthly_sales.aggregate(Sum('sale_price'))['sale_price__sum'] or 0,
    ...
    top_traders = monthly_sales.values(
...
def profit_analysis(request):
    ...
    if form.is_valid():
        ...
        if group_by == 'month':
            ...
            monthly_data = defaultdict(lambda: {
                'sales_count': 0, 'revenue': 0, 'margin': 0, 'commission': 0
            })
            ...
            grouped_data = [
                {
                    ...
                    'margin_percentage': (data['margin'] / data['revenue'] * 100) if data['revenue'] > 0 else 0,
                }
            ...
            for sale in sales:
                trader_name = sale.assigned_trader.get_full_name() or sale.assigned_trader.username
    ...
    context = {
        ...
        'sales_list': sales[:20] if not isinstance(sales, list) else sales[:20],  # Show first 20 for detail
    }
...
def inventory_status(request):
    ...
    slow_moving = [v for v in vehicles_list if v.days_in_stock > 90 and v.status == 'available']
...
def sales_summary(request):
    ...
    if form.is_valid():
        ...
        if period_type == 'monthly':
            ...
            from collections import defaultdict
            monthly_data = defaultdict(lambda: {
                'sales_count': 0, 'revenue': 0, 'commission': 0
            })
            ...
            period_data = [
                {
                    ...
                    'revenue': data['revenue'],
                    'commission': data['commission'],
                    'avg_sale': data['revenue'] / data['sales_count'] if data['sales_count'] > 0 else 0,
                }
                for month, data in sorted(monthly_data.items())
            ]
...
def payment_status(request):
    ...
    top_outstanding = invoices.filter(
        balance_due__gt=0
    ).values(
        'customer__name'
    ).annotate(
...
def export_to_excel(request, report_type):
    ...
    if report_type == 'profit_analysis':
        ...
        ws.append(['Date', 'Véhicule', 'Client', 'Prix de Vente', 'Marge', 'Commission'])
...
def export_to_csv(request, report_type):
    ...
    if report_type == 'profit_analysis':
        writer.writerow(['Date', 'Véhicule', 'Client', 'Prix de Vente', 'Marge', 'Commission'])
...
def export_to_pdf(request, report_type, include_charts=False):
    ...
    response['Content-Disposition'] = f'attachment; filename="rapport_{report_type}.html"'
...
def send_report_email(email_to, attachment, report_type, format_type):
    ...
    body = f"Veuillez trouver ci-joint le rapport {report_type} au format {format_type}."
...
def ajax_chart_data(request):
    ...
    if chart_type == 'monthly_sales':
        ...
        status_data = Vehicle.objects.values('status').annotate(
            count=Count('id')
        )
